...
const CompletedCountChart: React.FC<CompletedCountChartProps> = ({
  day_name_counts,
}) => {
  // Prepare the data for the chart, only including day counts
  const chartData = Object.keys(day_name_counts).map((name) => ({
    // Return Completed or Not Completed if name is true or false
    name: name === "true" ? "Completed" : "Not Completed",
    ...
    <Card className="flex flex-col">
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          ...
          <CardDescription>
            Analysis of maintenance tickets by status
          </CardDescription>
        </div>
      </CardHeader>
...
export default CompletedCountChart;
