...
const AddDowntimeToProductionLog: React.FC<AddDowntimeToProdLogProps> = ({
  open,
  onClose,
  log,
}) => {
  ...
  const mutation = useMutation({
    mutationFn: (data: DowntimeCreate) =>
      ProductionLogsService.createProductionDowntimeApiV1ProductionLogsDowntimesPost(
        {
          requestBody: data,
          // @ts-ignore
  ...
  const onSubmit: SubmitHandler<DowntimeCreate> = (data) => {
  ...
  return (
    <>
      <AddFormModal
        open={open}
        onClose={onClose}
        title={`Add downtime to ${log.log_number}`}
...
export default AddDowntimeToProductionLog;
